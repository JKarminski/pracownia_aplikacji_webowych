...
import { fetchSubtopicById } from "../api";

export default function Subtopic(): React.JSX.Element {
  ...
  const { data: sub, isLoading, isError } = useQuery({
    queryKey: ['subtopic', postId, subId],
    queryFn: () => fetchSubtopicById(postId, subId),
    enabled: !!(postId && subId),
  });

  if (isLoading) {
    return (
      <section className="container">
        <h1>Ładowanie tematu...</h1>
      </section>
    );
  }
  ...
  return (
    <article className="container">
      ...
      <div style={{ marginTop: 32 }}>
        <Link className="backLink" to={`/post/${sub.post.id}`}>← Powrot do kategorii</Link>
      </div>
    </article>
  );
}
